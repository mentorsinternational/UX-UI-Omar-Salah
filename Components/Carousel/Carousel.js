class Carousel {
	...
	cycleLeft() {
		...
		TweenMax.to(this.itemList[this.index], 0, { delay: 0.8, x: 0, display: 'none' });
		...
		this.data !== undefined
			...
			: null;
		TweenMax.to(this.itemList[this.index], 2, {
			display: 'block',
			delay: 0.8,
			opacity: 1,
			onComplete: () => {
	...
	cycleRight() {
		...
		TweenMax.to(this.itemList[this.index], 1.5, { x: 1500 });
		TweenMax.to(this.itemList[this.index], 0, { delay: 0.8, display: 'none', x: 0 });
...
class Card {
	...
	cycleLeft() {
		...
		TweenMax.to(this.card, 0, { delay: 0.8, display: 'none', x: 0, opacity: '0' });
		TweenMax.to(this.card, 1.5, { display: 'block', delay: 0.8, opacity: 1 });
	}

	cycleRight() {
		TweenMax.to(this.card, 1.5, { x: 1500 });
		TweenMax.to(this.card, 0, { delay: 0.8, display: 'block', x: 0, opacity: 0 });
		TweenMax.to(this.card, 2, { display: 'block', delay: 0.8, opacity: 1 });
	}
}
